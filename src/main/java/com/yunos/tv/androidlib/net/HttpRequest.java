...
import com.yunos.tv.androidlib.cache.CacheManager;
import com.yunos.tv.androidlib.util.BaseUtils;
import com.yunos.tv.androidlib.util.FrameConstants;
...
public class HttpRequest implements Runnable {
	private final static String cookiePath = "/data/data/com.yunos.tv.tvassistant/cache/cookie";
	// 区分get还是post的枚举
		...
		try {
			...
			if ((requestCallback != null)) {
				if (statusCode == HttpStatus.SC_OK) {
					...
					if (responseInJson.getSuccess()=="false") {
...
//						} else
						{
							handleNetworkError(responseInJson.getMessage());
						}
					} else {
						// 把成功获取到的数据记录到缓存
						if (urlData.getNetType().equals(REQUEST_GET)
						...
						handler.post(new Runnable() {
							...
							@Override
							public void run() {
								requestCallback.onSuccess(finalStrResponse);
							}
